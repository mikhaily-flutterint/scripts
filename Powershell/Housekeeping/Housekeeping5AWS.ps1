...
function EmailNotification {
    ...
    DECLARE @V_Recipients NVARCHAR(MAX) = 'dwhinfraalerts@flutterint.com'
...
$AllServers = Invoke-DbaQuery -SqlInstance $ControlServer -Query "SELECT DISTINCT ServerName server_name FROM landscape.dbo.housekeeping_control WHERE AWS_Active = 1"
...
$Houses = Invoke-DbaQuery -SqlInstance $ControlServer -Database "landscape" -Query "SELECT DISTINCT PathRoot FROM landscape.dbo.housekeeping_control WHERE AWS_Active = 1;"
Write-Host $Houses
foreach ($House in $Houses) {
    Write-Host "Now working with :" $House.PathRoot
    $NWResult = Get-ChildItem -Path $House.PathRoot -Recurse | Where-Object { -not $_.PSIsContainer}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUcjCWNyB/Auxn34yNckZ+xEut
# qCegggrdMIIFDDCCAvSgAwIBAgITFwAAAANoHQGylzOSswAAAAAAAzANBgkqhkiG
...
# DjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQysAaQURNTt+q34BnF+zzU
# w/vvCzANBgkqhkiG9w0BAQEFAASCAQAco74JTqDxbVLlgekallKvoH13HXwamAaU
# eicfEieMvRQjHCu5MMegt/k15UjckOT/zEpZLx0SGuaI0QA3iuKGqKnHXp8zL4tq
# Q3TnwBL15/+D5dCAKEE8DoGaAbc9ihhflOYNssudAz4pH5g7XCjwskXfe1bwPfz6
# 0POTDVTiEPaF6ltFYFtuUoRqU1/4SNJNe86DI7u9ioTMC2GUXVFHC2IS5nGJUzzd
# ze2gy344vg0keaHLcZ8yjtS59zLQd520uksOM1BFWTQIguGPddHpoEQST0TsfKKj
# w9F6nLnUsrucSv9KCSkAD7ohhbSs04pd7s0goTjRWxPMcHrs4W5c
