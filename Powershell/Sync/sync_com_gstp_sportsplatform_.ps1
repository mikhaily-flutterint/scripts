$source = 'IMDGNDWSQLP78'
$destination = 'IMDGNDWSQLU60'
$database = 'Staging'
$prefix = 'com_gstp_sportsplatform_'

$tables = Invoke-DbaQuery -SqlInstance $source -Database $database -Query "SELECT s.name + '.' + t.name name
FROM sys.tables      t
    JOIN sys.schemas s
        ON s.schema_id = t.schema_id
WHERE 1=1
AND t.type = 'U' 
AND t.name LIKE @var_prefix + '%'" -SqlParameters @{"var_prefix"=$prefix} -As DataTable

foreach ($table in $tables)
    {      
    $max_inserted_ts_query = 'SELECT ISNULL(MAX(inserted_ts),(DATEADD(MONTH,-1,GETDATE()))) max_inserted_ts FROM ' + $table.name
    $max_inserted_ts = Invoke-DbaQuery -SqlInstance $destination -Database $database -Query $max_inserted_ts_query -As SingleValue | Get-Date -Format “yyyy-MM-dd HH:mm:ss:fff”
   
    $select_query = 'SELECT * FROM ' + $database + '.'+ $table.name + ' WHERE inserted_ts > ''' + $max_inserted_ts + ''''
    ...
    Clear-Variable -Name max_inserted_ts, select_query
}
# SIG # Begin signature block
# MIINcwYJKoZIhvcNAQcCoIINZDCCDWACAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUsbSyIUzPi5Hlt071/vJhdGz9
# JiygggrdMIIFDDCCAvSgAwIBAgITFwAAAANoHQGylzOSswAAAAAAAzANBgkqhkiG
...
# DjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQJFznVzDQKziiAqrBhjCVS
# iC0uAzANBgkqhkiG9w0BAQEFAASCAQClm+MO1AFuB0uc2wapige4NH/ek+V92YNS
# AB5RZABkKlPwVMgSQ7b8rj2IgaDVNmoBoSNqRnC4cqKFQ487KQ7l/iocfs9pwtSa
# GTBwm2z/JNv95QKvKVa9qVgp08Uj0YLnQoYKLMx33N1ZU8nyhq98p3tqrM6K3PJL
# hX0b3L4x+8yBmTBVJxltEL/cOtAvQMfyPetKA4iy0ANUazOy0TR9pFF2XQFdB/Q7
# FqtdzPggUcgauqyI7WGM2k1Pqy6ttzS7OWS8Vdion4w9v9VWQiA5sqwPpMzzgp0o
# yc4LLMPBrCX9OM1AulKhbpSaxNgwwL613cKekfQtMwXwpu5v8Z7L
